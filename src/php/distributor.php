...
if (isset($_POST['distributor_register'])) {
    ...
    $isVerified = $_POST['status'];

    $role_id = 3;

    // Insert user data into tbl_user
    $sql = "INSERT INTO user1 (RoleID, username, password) VALUES ('$role_id', '$username', '$password')";
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (mysqli_query($conn, $medicine)) {
                ...
                // Insert additional data into the tbl_distributor table
                $distributor_sql = "INSERT INTO tbl_distributor (user_id, distributor_name, address, contact_number, status) 
                                   VALUES ('$user_id', '$distributor_name', '$address', '$contact_number', '$isVerified')";
